...
def sign_up():
    if request.method == 'POST':
        ...
        sql_query = "select email from fan  where email = %s;"
        val = (email,)
        ...
        if len(records) > 0:
            ...
            print(age_final)
            sql1 = "insert into `fan` values (%s,%s,%s,%s,%s,%s,%s)"
            val = (
                email, UserName, gender[0], int(age_final), birthdate,
                favoriteteam, str(generate_password_hash(password1, method='sha256')))
            print(val)
            try:
